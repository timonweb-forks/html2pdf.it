...
var paperFormatRegExp = /^(((\d)|(\d\.\d))+(in|cm|mm|px))\*(((\d)|(\d\.\d))+(in|cm|mm|px))$/;
...
module.exports = function (app) {
	app.get('/', function (req, res, next) {
		...
		if(FORMATS.indexOf(paperFormat) === -1 && !paperFormatRegExp.test(paperFormat)){
			return res.status(400).send(format('Invalid format, the following are supported: %s or you need to specify a custom paper size using the following format: paperwidth*paperheight, examples: "5in*7.5in"', FORMATS.join(', ')));
		}
		// var customPaperFormat = req.query.custom_format || false;
		// if(!customPaperFormatRegExp.test(customPaperFormat)) {
		// 	return res.status(400).send(format('Invalid custom paper format, please specify it in the following format: paperwidth*paperheight, examples: "5in*7.5in"'));
		// }
		...
		request.head(url, function (err, resp) {
			...
			generatePdfViaChromium();

			function generatePdfViaChromium() {
				...
				var pdfProcess = spawn('electron-pdf', options);
